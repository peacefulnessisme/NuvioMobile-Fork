    ...
    when (activeEngine) {
        ...
        ResolvedAndroidPlaybackEngine.Libmpv -> {
            ...
            LibmpvPlayerSurface(
                ...
                forceSdrToneMapping = playerSettings.androidForceSdrToneMapping,
                onControllerReady = onControllerReady,
                onSnapshot = onSnapshot,
                onError = onError,
            )
    ...
    AndroidView(
        ...
        factory = { viewContext ->
            NuvioLibmpvView(
                ...
                forceSdrToneMapping = forceSdrToneMapping,
            ).apply {
                layoutParams = android.view.ViewGroup.LayoutParams(MATCH_PARENT, MATCH_PARENT)
                keepScreenOn = false
                runCatching {
...
private class NuvioLibmpvView(
    ...
    private val forceSdrToneMapping: Boolean,
    ...
    override fun initOptions() {
        ...
        if (forceSdrToneMapping) {
            // Forces HDR (PQ/HLG) sources to be tone-mapped down to an SDR-safe
            // signal, for screens/panels that don't support HDR and would
            // otherwise show HDR video too dark or washed out.
            mpv.setOptionString("target-colorspace-hint", "no")
            mpv.setOptionString("target-prim", "bt.709")
            mpv.setOptionString("target-trc", "srgb")
            mpv.setOptionString("target-peak", "203")
