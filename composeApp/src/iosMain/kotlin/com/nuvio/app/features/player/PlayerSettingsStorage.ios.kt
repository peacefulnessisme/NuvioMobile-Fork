...
actual object PlayerSettingsStorage {
    ...
    private const val androidForceSdrToneMappingKey = "android_force_sdr_tone_mapping"
    ...
    actual fun saveSubtitleCustomFontPath(fontPath: String?) {
        ...
        if (fontPath.isNullOrBlank()) {
            ...
            NSUserDefaults.standardUserDefaults.setObject(fontPath, forKey = key)
        }
    }
    ...
    actual fun loadAndroidForceSdrToneMapping(): Boolean? {
        val defaults = NSUserDefaults.standardUserDefaults
        val key = ProfileScopedKey.of(androidForceSdrToneMappingKey)
        return if (defaults.objectForKey(key) != null) {
            defaults.boolForKey(key)
        } else {
            null
    ...
    actual fun saveAndroidForceSdrToneMapping(enabled: Boolean) {
        NSUserDefaults.standardUserDefaults.setBool(enabled, forKey = ProfileScopedKey.of(androidForceSdrToneMappingKey))
    }
    ...
    actual fun exportToSyncPayload(): JsonObject = buildJsonObject {
        ...
        loadAndroidForceSdrToneMapping()?.let { put(androidForceSdrToneMappingKey, encodeSyncBoolean(it)) }
    ...
    actual fun replaceFromSyncPayload(payload: JsonObject) {
        ...
        payload.decodeSyncBoolean(androidForceSdrToneMappingKey)?.let(::saveAndroidForceSdrToneMapping)
